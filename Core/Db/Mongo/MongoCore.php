...
class MongoCore
{
     ...
     * @return void
     */
    public function setConfig($dbname, $config = array())
    {
        if (is_array($config)) {
     ...
     * @return boolean
     */
    public function switchDb($dbname, $dsn = '')
    {
        if (!empty($dsn)) {
        ...
        try {
            ...
            throw new \Exception('Unable to switch Mongo Databases: ' . $Exception->getMessage());
        }
    }
        ...
        if (empty($database)) {
           throw new \Exception('Failed to drop MongoDB database because name is empty');
            ...
            try {
                ...
                throw new \Exception('Unable to drop Mongo database `' . $database . '`: ' . $Exception->getMessage());
    ...
    */
    ...
    public function addIndex($collection = '', $fields = array(), $options = array())
    {
        ...
        if (empty($fields) OR ! is_array($fields)) {
            throw new \Exception('Index could not be added to MongoDB collection because no keys were specified');
        }
    ...
    public function removeIndex($collection = '', $keys = array())
    {
       ...
       if ($this->_dbhandle->{$collection}->deleteIndex($keys)) {
            ...
            // @codeCoverageIgnoreStart
            throw new \Exception('An error occurred when trying to remove an index from MongoDB Collection');
            // @codeCoverageIgnoreEnd
        }
     ...
     * @return void
     */
    private function _connectionString($dbname)
    {
